...
bot.onText(/\/start/, (msg) => {
	bot.sendMessage(
			msg.chat.id,
			'Привіт! Я бот, який допоможе вам з утилізацією відходів. Введіть тип відходу, і я надам вам інформацію про сортування.'
	);
});
...
async function getSortingInfoFromGPT(wasteType) {
	const messages = [
		{ role: "user", content: `Надай інформацію двома реченнями про те, чи можна сортувати, та куди класти такий тип сміття, як "${wasteType}"`}
	];
	...
	try {
		...
		const sortingInfo = response.data.choices[0].message.content.trim();
		return sortingInfo;
	} catch (error) {
